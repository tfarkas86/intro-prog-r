...
2. Calculate the mean plant height for all plants at each of the three sampling time points. Produce a named vector `pd.means` (ie `names(x)` not == `NULL`) of length == 3. 
...
8. Below is the code for linear regressions `?lm()` to test the same hypotheses about plant height, plant biomass, and fertilization. 

Predict how the results will differ from the t-tests above.
