...
typeof(y)
z <- x * y
type <- c("x" = typeof(x), "y" = typeof(y), "z" = typeof(z)); type
```
...
y2 <- y^2; y2 # 1
x_y2 <- y2 >= x; x_y2 # 2
x_y2.500 <- y2 >= x & y2 < 500; x_y2.500 # 3
x_y2.200 <- y2 >= x & y2 > 200; x_y2.200 # 4
sum(x_y2.200) == sum(x_y2.500) # 5
x_y2.comp <- x_y2.500 != x_y2.200; x_y2.comp # 6
sum(x_y2.comp) == 0 # 7 
identical(x_y2.500, x_y2.200) # 7 alternative
z.comp <- z[x_y2.comp]; z.comp # 8
```
Problem 2.3
...
```{r}
d1 <- sample(s1, 20); d2 <- sample(s2, 20)
```
...
```{r}
sum_stats <- c(d1_mean=mean(d1), d2_mean=mean(d2))
```
...
```{r}
sum_stats[c("d1_var", "d2_var")] <- c(var(d1), var(d2))
```
...
```{r}
man_var <- c(d1_var = sum((d1 - mean(d1)) ^ 2) / length(d1), 
             d2_var = sum((d2 - mean(d2)) ^ 2) / length(d2))
```
...
```{r}
var_var <- c(sum_stats["d1_var"] == man_var[1], 
             sum_stats["d2_var"] == man_var[2])
```

The function `t.test()` can evaluate the probability that two sets of data (ie samples) are generated using the same process (ie drawn from the same population). This probability is expressed in the output of `t.test()` as the "p-value".
