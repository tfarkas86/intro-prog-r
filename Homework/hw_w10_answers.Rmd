...
```{r 10.2.1-2}
wd <- read.csv("~/Dropbox/UNM R Course/warming.csv")
wd$diff <- wd$warm - wd$cntl
```

```{r 10.2.3-4}
sapply(wd$diff, FUN = function(i) { # loop through all values in diff
  
  sapply(wd$diff, FUN = function(j) { # loop through all values in diff
...
# more verbose (= bad) alternative uses i and j to index diff

sapply(1:length(wd$diff), FUN = function(i) {
  
  sapply(1:length(wd$diff), FUN = function(j) {
...
```{r 10.2.5}
sapply(wd$diff, FUN = function(i) {
  
  sapply(wd$diff, FUN = function(j) 0.5 * (i - j) ^ 2)
  
})
```

```{r 10.2.6}
diffmat <- sapply(wd$diff, FUN = function(i) {
  
  sapply(wd$diff, FUN = function(j) 0.5 * (i - j) ^ 2)
  
})

pv <- sum(diffmat) / length(wd$diff) ^ 2
```
...
pop.var <- function(x) {
  ...
  diffmat <- sapply(x, FUN = function(i) {
  
  sapply(x, FUN = function(j) 0.5 * (i - j) ^ 2)
  
})
  
  pv <- sum(diffmat) / length(x) ^ 2
  return(list(dist=diffmat, pvar = pv))
}
...
1. Choose an appropriate looping function (`lapply()`, `sapply()`, etc.) to call `pop.var()` on all unique subsets of `diff` (from warming.csv) that exclude a single value. Your function should return only a vector of `pvar` values named `pvars`, so you will need to index the list returned by `pop.var` to exclude the half-squared distance matrix named `dist`.
...
```{r 10.3.1-5}
pvars <- sapply(1:length(wd$diff), FUN=function(i) pop.var(wd$diff[-i])$pvar)

var.diff <- mean(pvars) - pop.var(wd$diff)$pvar
bias <- (length(wd$diff) - 1) * var.diff

pop.var(wd$diff)$pvar - bias

var(wd$diff)
```
```{r 10.3.6}
pop.var <- function(x, pop=TRUE) {
  
  div <- ifelse(pop, length(x) ^ 2, (length(x) ^ 2 - length(x)))
...
```
```{r 10.3.7}
pvars <- sapply(1:length(wd$diff), FUN=function(i) pop.var(wd$diff[-i], pop=FALSE)$pvar)

var.diff <- mean(pvars) - pop.var(wd$diff, pop=FALSE)$pvar
bias <- (length(wd$diff) - 1) * var.diff
bias # no bias!

pop.var(wd$diff, pop=FALSE)$pvar
var(wd$diff) # the same
```
