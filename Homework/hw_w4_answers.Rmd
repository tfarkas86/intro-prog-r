...
y <- rpois(100, 100)
matrix(y, nrow=10) # 1
ymat <- matrix(y, nrow=20, byrow=TRUE) # 2
dimnames(ymat) <- list(1:20, LETTERS[1:ncol(ymat)]) # 3
t(t(ymat)) # 4
ymat[5, 2] # 5
ymat[2, 5] # 6
ymat[2,  ] # 7
...
```{r}
ymat[ymat < mean(ymat)] # 1
ymat[ymat < 90 | ymat > 110] # 2
ymat[ymat < 90 | ymat > 110 | ymat == 100] # 3
ymat[ymat %% 2 == 0]; # 4
ymat[(ymat < 90 | ymat > 110) & ymat %% 2 == 0] # 5
...
```{r}
ymat[ymat[, 1] > 100, ] # 1
ymat[ , ymat[5, ] < 95 | ymat[5, ] > 105, drop=FALSE] # 2
ymat[rowSums(ymat) > 500, ] # 3
ymat[ , colMeans(ymat) < 500]
```
...
```{r}
iris.se <- iris[iris$Species == "setosa", 1:4] # 1
hist(iris.se$Sepal.Length) # 2
i.s.m <- colMeans(iris.se) # 3
iris.se[iris.se$Sepal.Length < i.s.m["Sepal.Length"], ] # 4
iris.se[iris.se$Sepal.Length < i.s.m["Sepal.Length"], "Sepal.Width"] # 5
iris.se$Sepal.Width[iris.se$Sepal.Length < i.s.m["Sepal.Length"]] # 5 alternate
iris.se[iris.se$Sepal.Length < i.s.m["Sepal.Length"] & # 6
        iris.se$Petal.Length > i.s.m["Petal.Length"], ] # 6 cont.
